...
def getorguplinklosslatency(apikey, orgid, uplink, suppressprint=False):
    calltype = 'Uplink Loss and Latency'
    geturl = '{0}/organizations/{1}/uplinksLossAndLatency?uplink={2}'.format(
        str(base_url), str(orgid), str(uplink))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def updatenetwork(apikey, networkid, tags, suppressprint=False):
